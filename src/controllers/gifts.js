    ...
    try {
      ...
      const gifts = data.map(gift => {
        ...
        concatanetedItems.forEach(pair => {
          ...
          if (pairArray[0] == gift.boxId) items.push({ title: pairArray[1], quantity: pairArray[2] });
        });
        return {
          id: gift.id,
          boxId: gift.boxId,
    ...
    const gifts = data.map(gift => {
      ...
      concatanetedItems.forEach(pair => {
        ...
        if (pairArray[0] == gift.boxId) {
          ...
          items.push({ title: pairArray[1], quantity: pairArray[2], price: pairArray[3]})
        }
      });
      return {
        id: gift.id,
